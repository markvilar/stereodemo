...
from stereodemo.methods import Config, InputPair, Calibration, StereoMethod

data_folder = (
    Path(__file__).parent.parent / "datasets" / "eth3d_lowres" / "delivery_area_1l"
)
...
class TestStereoInference(unittest.TestCase):
    ...
    def check_method(
        self, method: StereoMethod, expected_median: float, expected_coverage: float
    ):
        output = method.compute_disparity(input)
        valid_pixels = output.disparity_pixels[output.disparity_pixels > 0.0]
