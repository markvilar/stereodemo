...
class ChangRealtimeStereoOnnx(StereoMethod):
        ...
        self.parameters.update(
            {
                "Shape": EnumParameter(
                    ...
                    1,
                    ["160x128", "320x240", "640x480", "1280x720"],
                )
            }
        )
    ...
    def compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        # NOTE: Adding this to suppress linter errors - figure out where the
        # argument comes from
        models_path: Path = Path("../resources/models")
        ...
        outputs = self._loaded_session.run(
            ["disparity"], {"left": left_tensor, "right": right_tensor}
        )
    ...
    def _preprocess_input(self, img: np.ndarray):
        ...
        img = (img.astype(np.float32) / 255.0) - imagenet_stats[
            "mean"
        ] / imagenet_stats["std"]
