...
class OakdSource(visualizer.Source):
    ...
    def connect(self):
        ...
        disparityIntrinsics = oak_calib.getCameraIntrinsics(
            dai.CameraBoardSocket.RIGHT, dai.Size2f(w, h)
        )
        baselineMeters = 1e-2 * oak_calib.getBaselineDistance(
            dai.CameraBoardSocket.LEFT, dai.CameraBoardSocket.RIGHT
        )

        self.calibration = visualizer.Calibration(
            w,
            h,
        ...
        self.disparityQueue = self.device.getOutputQueue(
            name="disparity", maxSize=1, blocking=False
        )
        self.rectifiedLeftQueue = self.device.getOutputQueue(
            name="rectifiedLeft", maxSize=1, blocking=False
        )
        self.rectifiedRightQueue = self.device.getOutputQueue(
    ...
    def get_next_pair(self):
        ...
        if self.output_folder is not None:
            ...
            cv2.imwrite(
                str(self.output_folder / f"img_{self.frameIndex:03d}_left.png"),
                leftFrame,
            )
            cv2.imwrite(
                str(self.output_folder / f"img_{self.frameIndex:03d}_right.png"),
                rightFrame,
            )
        ...
        return visualizer.InputPair(
            leftFrame, rightFrame, self.calibration, "OAK-D Camera", disparityPixels
        )
...
class StereoFromOakInputSource(StereoMethod):
    def __init__(self, config: Config):
        super().__init__(
            "Input Source", "Stereo computed by the input source", {}, config
        )
