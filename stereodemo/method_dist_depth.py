...
class DistDepth(StereoMethod):
    ...
    def compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        device = (
            torch.device("cuda") if self.parameters["Device"].value == "CUDA" else "cpu"
        )

        model_path = self.config.models_path / "dist-depth-256x256.scripted.pt"
