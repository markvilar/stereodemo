...
class CREStereo(StereoMethod):
    ...
    def reset_defaults(self):
        self.parameters.update(
            {
                "Iterations": EnumParameter(
                    "Number of iterations", 1, ["2", "5", "10", "20"]
                ),
                "Mode": EnumParameter(
                    "Number of passes. The combined version does 2 passes, one to get an initial estimation and a second one to refine it.",
                    1,
                    ["init", "combined"],
                ),
                "Shape": EnumParameter(
                    "Processed image size", 1, ["320x240", "640x480", "1280x720"]
                ),
            }
        )
    ...
    def compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        model_path = (
            ...
            / f"crestereo_{version}_iter{iters}_{rows}x{cols}.onnx"
        )
        ...
        if self._has_flow:
            ...
            start = time.time()
            outputs = self._inference_with_flow(
                left_tensor_half, right_tensor_half, left_tensor, right_tensor
            )
        else:
    ...
    def _inference_without_flow(self, left_tensor, right_tensor):
        ...
        return self._loaded_session.run(
            self.output_names,
            {self.input_names[0]: left_tensor, self.input_names[1]: right_tensor},
        )[0]

    def _inference_with_flow(
        self, left_tensor_half, right_tensor_half, left_tensor, right_tensor
    ):
