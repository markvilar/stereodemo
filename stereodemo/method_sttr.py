...
class StereoTransformers(StereoMethod):
        ...
        self.parameters.update(
            {
                ...
                "Model": EnumParameter(
                    "Pre-trained Model", 0, ["kitti-cpu", "sceneflow-cpu"]
                ),
                "Detect occlusions": EnumParameter(
                    "Detect Occlusions", 0, ["Yes", "No"]
                ),
            }
        )
    ...
    def _compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        model_path = (
            ...
            / f"sttr-{variant}-{rows}x{cols}-ds{downsample}.scripted.pt"
        )
