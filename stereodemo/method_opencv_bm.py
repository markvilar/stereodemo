...
class StereoBM(StereoMethod):
    ...
    def reset_defaults(self):
        ...
        self.parameters.update(
            {
                "Num Disparities": IntParameter(
                    "Number of disparities (pixels)",
                    128,
                    16,
                    640,
                    to_valid=multiple_of_16,
                ),
                "Block Size": IntParameter(
                    "Kernel size for block matching (odd)", 9, 3, 63, to_valid=odd_only
                ),
                "TextureThreshold": IntParameter(
                    "Minimum SAD to consider the texture sufficient", 10, 0, 100
                ),
                "Uniqueness Ratio": IntParameter(
                    "How unique the match each for each pixel", 15, 0, 100
                ),
                "SpeckleWindowSize": IntParameter(
                    "Speckle window size in pixels (filter small objects). 0 to disable.",
                    100,
                    0,
                    1000,
                ),
                "SpeckleRange": IntParameter(
                    "Speckle range (max diff within a window)", 32, 0, 64
                ),
                "Disp12MaxDiff": IntParameter(
                    "Maximum allowable difference in the right-left check", 1, 0, 64
                ),
                "PreFilterCap": IntParameter("Max pre-filter output", 31, 1, 63),
                "PreFilterSize": IntParameter(
                    "Pre-filter size (odd)", 9, 5, 255, to_valid=odd_only
                ),
            }
        )
    ...
    def compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        stereoBM = cv2.StereoBM_create(
            numDisparities=self.parameters["Num Disparities"].value,
...
class StereoSGBM(StereoMethod):
    ...
    def reset_defaults(self):
        ...
        self.parameters.update(
            {
                "Num Disparities": IntParameter(
                    "Number of disparities (pixels)", 128, 2, 640
                ),
                "Block Size": IntParameter(
                    "Kernel size for block matching (odd)", 3, 3, 63, to_valid=odd_only
                ),
                "Mode": EnumParameter(
                    ...
                    0,
                    ["MODE_SGBM", "MODE_HH", "MODE_SGBM_3WAY", "MODE_HH4"],
                ),
                "P1": IntParameter(
                    "Penalty Cost (default=8*NChannels*BlockSize)",
                    8 * nchannels * 3 * 3,
                    0,
                    2000,
                ),
                "P2": IntParameter(
                    "Penalty Cost. Must be > P1 (default=32*NChannels*BlockSize).",
                    32 * nchannels * 3 * 3,
                    0,
                    2000,
                ),
                "Uniqueness Ratio": IntParameter(
                    "How unique the match each for each pixel", 10, 0, 100
                ),
                "SpeckleWindowSize": IntParameter(
                    "Speckle window size in pixels (filter small objects). 0 to disable.",
                    100,
