...
def parse_args():
    ...
    parser.add_argument(
        "--oak", action="store_true", help="Use an oak-D camera to grab images."
    )
    parser.add_argument(
        "--oak-output-folder",
        type=Path,
        default=None,
        help="Output folder to save the images grabbed by the OAK camera",
    )
    parser.add_argument(
        "images",
    ...
    parser.add_argument(
        "--calibration",
        type=Path,
        help="Calibration json. If unspecified, it will try to load a stereodemo_calibration.json file in the left image parent folder.",
        default=None,
    )
    ...
    def get_pair_at_index(self, idx: int) -> methods.InputPair:
        ...
        if self.user_provided_calibration_path is None:
            ...
            if not calibration_path.exists():
                print(
                    f"Warning: no calibration file found {calibration_path}. Using default calibration, the point cloud won't be accurate."
                )
        ...
        return visualizer.InputPair(
            left_image, load_image(right_image_path), calib, status
        )
...
def main():
    ...
    try:
        ...
        sys.stderr.write(
            f"Warning: cannot use the default models path {args.models_path}: {e}\n"
        )
        ...
        sys.stderr.write(
            f"Going to use the temporary directory {args.models_path} instead, specify --model-paths to specify a custom persistent path instead.\n"
        )
        try:
            args.models_path.mkdir(parents=True, exist_ok=True)
        except BaseException:
            sys.stderr.write(
                "Could not create a temporary directory to store the downloaded models.\n"
            )
            sys.stderr.write(
                "Aborting, you need to specify --models-path with a valid writable path.\n"
            )
            sys.exit(1)
